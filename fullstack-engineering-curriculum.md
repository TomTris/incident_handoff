...
Be ready to discuss: "Design an incident management system for 100 teams."
- Go microservices per domain (incident API, notification service, schedule service)
- PostgreSQL for persistence, Redis for pub/sub between instances
- WebSocket gateway for real-time
- Kubernetes deployment with horizontal pod autoscaling
- Prometheus + Grafana for monitoring
- Feature flags for progressive rollout

You built a miniature version of this. Scale the explanation up.

### Go Technical Questions

Prepare concrete answers:
- Goroutines vs threads — cost (~2KB vs ~8MB), M:N scheduling, growable stacks
- Channels vs mutex — when each is appropriate (cite your Phase 3 experience)
- `sync.Pool` — what it does, when to use it (cite Phase 4 benchmark results)
- `context.Context` — cancellation, timeout, value propagation through the call chain
- Interface satisfaction — implicit, what happens when you add a method to an interface
- Connection pooling — how `pgxpool` manages connections, why it matters under load
- Go's garbage collector — how allocations affect GC pause, what `sync.Pool` does to help

### Frontend Technical Questions

Prepare concrete answers:
- Composition API vs Options API — why Composition (better TypeScript, composability)
- `ref` vs `reactive` — `ref` for primitives, `reactive` for objects (but `ref` works for both)
- Pinia — why centralized state, how it compares to prop drilling
- WebSocket reconnection — exponential backoff, max delay, cleanup on unmount
- Four-state UI — loading, error, empty, data — why all four matter

### YBIYRI Questions

"Tell me about a system you built and ran."
— Handoff. You built the API, the database schema, the WebSocket layer, the auth system, the metrics instrumentation, the frontend, the Docker setup, the CI pipeline, the Kubernetes manifests, and the deployment. You know what every `/metrics` counter tracks and what every Prometheus histogram bucket measures. You own every layer.

### General

"Why should we consider you?"
— You compressed a 3.5-year degree into 1.5 years. You completed 42's core curriculum solo in 6 months. You built Handoff end-to-end in the exact stack the team uses. You learn fast and you ship. The commit history proves it.

---

*Start Phase 5 today. Initialize the repo. First commit is the project scaffolding.*
